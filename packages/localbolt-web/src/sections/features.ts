...
export function createFeatures(): HTMLElement {
  ...
  const features = [
    ...
    { icon: icons.laptop, title: 'Browser &amp; Desktop App', desc: 'Use localbolt.app in any browser or <a href="/download/macos" class="text-neon/70 hover:text-neon transition-colors underline underline-offset-2">download the native desktop app</a> for macOS. Apple Silicon · macOS 14+. <span class="block mt-1.5 text-[10px] text-gray-600">Ad-hoc signed — right-click → Open on first launch. <a href="https://github.com/the9ines/localbolt-native/releases/tag/localbolt-app-v2.0.0" target="_blank" rel="noopener noreferrer" class="underline hover:text-gray-400 transition-colors">Release notes &amp; checksum</a></span>' },
