...
### I4. Static shell generation
- **Resolved:** `packages/localbolt-web` now generates a static HTML shell
  after the Vite client bundle.
- This is build-time static site generation, not Netlify legacy prerendering
  and not a runtime SSR server.
- Netlify remains the sponsored static deployment host.
- The generated shell must stay LAN-only in visible copy and structured data.
